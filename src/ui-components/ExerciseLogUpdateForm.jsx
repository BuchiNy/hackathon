...
import { getExerciseLog } from "../graphql/queries";
import { updateExerciseLog } from "../graphql/mutations";
const client = generateClient();
export default function ExerciseLogUpdateForm(props) {
  const {
    id: idProp,
    exerciseLog: exerciseLogModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = exerciseLogRecord
      ? { ...initialValues, ...exerciseLogRecord }
      : initialValues;
    setPatientID(cleanValues.patientID);
    setPlanID(cleanValues.planID);
    setPlanItemID(cleanValues.planItemID);
    setCompletedAt(cleanValues.completedAt);
    setFormScore(cleanValues.formScore);
    setErrors({});
  };
  const [exerciseLogRecord, setExerciseLogRecord] =
    React.useState(exerciseLogModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getExerciseLog.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getExerciseLog
        : exerciseLogModelProp;
      setExerciseLogRecord(record);
    };
    queryData();
  }, [idProp, exerciseLogModelProp]);
  React.useEffect(resetStateValues, [exerciseLogRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateExerciseLog.replaceAll("__typename", ""),
            variables: {
              input: {
                id: exerciseLogRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ExerciseLogUpdateForm")}
      {...rest}
    >
      <TextField
        label="Patient id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || exerciseLogModelProp)}
